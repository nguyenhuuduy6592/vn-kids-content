...
import { Plus, Search, Star, Archive, BookOpen, Music, FileText, Shuffle, X, Check, Moon, Sun, RotateCcw, Eye, Loader, RefreshCw, Upload, Download, ExternalLink } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className={`relative w-full max-w-lg max-h-[85vh] ${bgCard} rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col`}>
        ...
        <div className={`p-4 border-t ${border} flex gap-2`}>
          ...
          <button onClick={() => window.open(`https://www.google.com/search?q=${encodeURIComponent(item.title)}`, '_blank')} className={`p-3 rounded-xl ${isDark ? "bg-zinc-800" : "bg-zinc-100"}`} title="Tìm trên Google"><ExternalLink size={20} className="text-blue-500" /></button>
        </div>
      </div>
    </div>
  );
